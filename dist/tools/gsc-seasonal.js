 ...
 */
async function gscSeasonal(includePartialMonths = false, dataset) {
  ...
  `;
    const result = await (0, query_js_1.runQuery)(sql, 100);
    const dropped = [];
    const rows = result.rows.filter((row) => {
        if (row.is_complete_month)
            return true;
        dropped.push(`${row.year}-${String(row.month).padStart(2, "0")} (${row.days_with_data} von ${row.days_in_month} Tagen)`);
