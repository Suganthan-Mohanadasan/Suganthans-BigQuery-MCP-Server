...
server.tool(
  "query",
  "Run a SQL query against BigQuery and return results. Only SELECT queries are allowed. Claude should use list_datasets, list_tables, and describe_table first to understand the schema before writing queries.",
  {
...
server.tool(
  "list_tables",
  "List all tables in a BigQuery dataset with their schemas. Use this to understand what tables and columns are available before writing queries.",
  {
...
server.tool(
  "gsc_quick_wins",
  "Find keywords from GSC bulk export data at positions 4 to 15 with high impressions. These are striking distance keywords that could be pushed to page one. Requires GSC bulk data export to BigQuery.",
  {
...
server.tool(
  "gsc_content_decay",
  "Find pages with consistent traffic decline over three consecutive months from GSC bulk export data. One bad month is noise; three is a problem. Requires GSC bulk data export to BigQuery.",
  {
...
server.tool(
  "gsc_cannibalisation",
  "Find keywords where multiple pages from your site compete against each other in GSC bulk export data. Shows which pages rank for the same query and their respective positions. Requires GSC bulk data export to BigQuery.",
  {
    days: z.number().default(28).describe("Number of days to analyse"),
...
server.tool(
  "gsc_traffic_drops",
  "Find pages that lost the most traffic recently using GSC bulk export data. Compares current period vs prior period and diagnoses whether each drop is a ranking loss, CTR collapse, or demand decline. Requires GSC bulk data export to BigQuery.",
  {
