...
const ARTEFACT_SRC =
  "^(yes|yeah|yep|ok|okay|okey|sure|no|nope|go on|continue|keep going|tell me more|more|next|why not|how so|really|exactly|show me|and then|thanks|thank you|got it|what else|anything else)[?!.,]*$";
const PIVOT_SRC = "^(what|how) about .{1,40}$";
const TRACKER_SRC = "(\\. ?my location is )|(^evaluate the .+ on )";
const AGENT_SRC =
  "^(search the web for|browse the web)|do not invent|return the [0-9]+ most relevant|include source urls";
const CONV_SRC =
  "\\b(please|can you|could you|give me|show me|tell me|i want|i need|i have|i am|should i|do i need|is it worth|how do i|how can i|help me|walk me through)\\b";
...
export function classifyQuery(query: string): Bucket | null {
  const q = query.toLowerCase().trim();
  const words = q.split(/ +/).length;
  if (RE.artefact.test(q)) return "artefact";
  if (RE.pivot.test(q)) return "pivot";
  if (RE.agent.test(q)) return "agent_harness";
  if (RE.tracker.test(q)) return "tracker_probe";
  if (RE.pasted.test(q)) return "pasted_string";
  if (RE.conv.test(q) || RE.deictic.test(q)) return "conversational";
  if (words >= 10 && QSTART.test(q)) return "conversational";
  if (words >= 10) return "long_uncategorised";
  return null;
}
...
const BUCKET_MEANING: Record<Bucket, string> = {
  artefact:
    "A person mid-conversation with Google's AI replied ('yes', 'go on') and the reply was logged as a query your page appeared for. Positions here are AI-block positions, not open-web rankings.",
  pivot:
    "Mid-conversation comparison questions ('what about resend'). Each one names the alternative users ask the AI about next; a ready-made comparison-section instruction for the landing page.",
  conversational:
    "Human questions shaped by AI conversation. High impressions with near-zero clicks here means your page is being read inside answers rather than clicked from results.",
  tracker_probe:
    "Synthetic daily prompts from AI-visibility tracking tools flowing through Google's grounding layer. Not human demand: exclude from opportunity analyses. If you run an AI tracker, this is its probes independently logged by Google; if you don't, third parties are sweeping topics you rank in.",
  agent_harness:
    "Complete agent instructions ('search the web for… do not invent results') logged as queries. Machine traffic; interesting for what agents surface you for, worthless as keyword demand.",
  pasted_string:
    "Error messages and CSV headers entered as searches, by people or pipelines.",
  long_uncategorised:
    "Ten or more words without another marker. A mix of quoted-sentence searches, agent fragments and rare long-tail; review manually before acting.",
};
...
interface BucketRow {
  ...
  ctr: number;
  position: number;
  top_pages: Array<{ page: string; impressions: number }>;
}

export async function gscGenaiConversationQueries(
  days: number = 365,
  ...
  const [main, iceberg, retention, timeline] = await Promise.all([
    runQuery(mainSQL, 2000),
    runQuery(icebergSQL, 10),
    runQuery(retentionSQL, 10),
    includeTimeline ? runQuery(timelineSQL, 200) : Promise.resolve(null),
  ]);

  const buckets: Record<Bucket, BucketRow[]> = {
    artefact: [],
    pivot: [],
    conversational: [],
    tracker_probe: [],
  ...
  for (const row of main.rows) {
    ...
    const topPages = Array.isArray(row.top_pages)
      ? (row.top_pages as Array<{ page: string; impressions: number }>).map((p) => ({
          page: String(p.page),
  ...
  const summary: Record<string, { queries: number; impressions: number; clicks: number }> = {};
  ...
  return {
    ...
    anonymised_visibility: {
      ...
      meaning:
        "Impressions whose query Google anonymised as too rare. Conversations are almost by definition rare strings, so most of the conversation iceberg sits in this pool; the buckets above are its visible tip.",
    },
